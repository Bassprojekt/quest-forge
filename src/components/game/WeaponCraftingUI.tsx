...
export const WeaponCraftingUI = ({ onClose }: Props) => {
  const inventory = useGameStore(s => s.inventory);
  const craftItem = useGameStore(s => s.craftItem);
  const [category, setCategory] = useState<'weapon' | 'armor'>('weapon');

  const filtered = CRAFTING_RECIPES.filter(r => r.type === category);
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center pointer-events-auto"
      ...
      <div className="relative bg-white/95 backdrop-blur-md border-2 border-[#E0D5C0] rounded-2xl p-6 max-w-lg w-full mx-4"
        ...
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-[#FF9800] font-bold text-lg">🔨 Handwerk</h2>
          <button onClick={onClose} className="text-[#AAA] hover:text-[#333] text-xl">✕</button>
        </div>

        <div className="flex gap-2 mb-4">
          <button onClick={() => setCategory('weapon')}
            className={`px-4 py-1.5 rounded-xl text-xs font-bold transition-colors ${
              category === 'weapon' ? 'bg-[#FF9800] text-white' : 'bg-[#F0F0F0] text-[#888] hover:text-[#333]'
            }`}>
            ⚔️ WAFFEN
          </button>
          <button onClick={() => setCategory('armor')}
            className={`px-4 py-1.5 rounded-xl text-xs font-bold transition-colors ${
              category === 'armor' ? 'bg-[#FF9800] text-white' : 'bg-[#F0F0F0] text-[#888] hover:text-[#333]'
            }`}>
            🛡️ RÜSTUNG
          </button>
        </div>
            ...
            <div key={recipe.id}
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-[#333] text-sm font-bold">{recipe.icon} {recipe.name}</span>
              ...
              <button onClick={() => canCraft(recipe) && craftItem(recipe.id)}
